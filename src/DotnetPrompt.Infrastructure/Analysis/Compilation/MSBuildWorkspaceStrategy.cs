        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            return new CompilationResult
            {
                Success = false,
                StrategyUsed = CompilationStrategy.MSBuild,
                ErrorMessage = $"MSBuild compilation failed: {ex.Message}",
                CompilationTimeMs = (long)(DateTime.UtcNow - startTime).TotalMilliseconds
            };
        }
        finally
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (targetProject == null)
        {
            return new CompilationResult
            {
                Success = false,
                StrategyUsed = CompilationStrategy.MSBuild,
                ErrorMessage = "No suitable C# project found in solution for analysis"
            };
        }
        ...
        if (compilation == null)
        {
            return new CompilationResult
            {
                Success = false,
                StrategyUsed = CompilationStrategy.MSBuild,
                ErrorMessage = $"Failed to create compilation from MSBuild project '{targetProject.Name}'"
            };
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (compilation == null)
        {
            return new CompilationResult
            {
                Success = false,
                StrategyUsed = CompilationStrategy.MSBuild,
                ErrorMessage = $"Failed to create compilation from MSBuild project '{project.Name}'. " +
